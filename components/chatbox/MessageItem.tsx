...
const MessageItem: React.FC<{
  ...
  const handleContextMenu = (e?: React.MouseEvent) => {
    ...
    const adjustedLeft =
      left + menuWidth > innerWidth ? innerWidth - menuWidth - 20 : left;
  ...
  return (
    ...
    >
      <div
        className={`relative aspect-square max-sm:h-[20px] max-sm:w-[20px] h-[30px] w-[30px]  ${
          message.sender._id !== userId ? "mr-2" : "ml-2"
        }`}
      >
        <img
      ...
      <div className="">
        <div
          className={`flex relative  w-fit  ${
            message.mediaUrl
              ? "min-w-[20rem] max-sm:min-w-[13rem]"
              : "min-w-[15rem] max-sm:min-w-[10rem] "
          } p-3 pt-0 rounded-lg  ${
            message.sender._id !== userId
              ? "bg-[#FF971D] shape rounded-tl-none"
              : "bg-primary-500 shape-2 text-[#ffffff] rounded-tr-none"
          }`}
        >
          <div className="flex flex-col relative w-full">
            <div className="flex w-full justify-between">
              <span className="font-semibold max-sm:text-[14px]">
                {message.sender.name}
              </span>
              <div className=" cursor-pointer " onClick={handleContextMenu}>
                <Image
            ...
            <div
              className={`flex flex-col relative w-full ${
                message.mediaUrl ? " p-5 pt-0" : " p-3 pt-0"
              }`}
            >
              {message.replyTo && (
              ...
              <div
                className={`text-xs absolute bottom-0 right-1 flex items-center space-x-1 ${
                  message.sender._id !== userId
                    ? "text-gray-500"
                    : "text-gray-300"
                }`}
              >
                <span
                  className={`text-xs ${
                    message.isDelivered ? "text-gray-300" : "hidden"
                  }`}
                >
                  ✓
                </span>
                <span
                  className={`text-xs ${
                    message.isRead ? "text-gray-300" : "hidden"
                  }`}
                >
                  ✓✓
                </span>
          ...
          {openMenuId === message._id && menuPosition && (
            <div
              ...
              style={{
                top:
                  index === totalMessages - 1 && totalMessages > 5
                    ? "50%"
                    : `230%`,
            ...
            >
              <button
                ...
                onClick={() => handleMenuOptionClick("Reply")}
              >
              ...
              <button
                className="flex items-center p-2 hover:bg-gray-100 rounded"
                onClick={() => handleMenuOptionClick("React")}
              >
              ...
              <button
                className="flex items-center p-2 hover:bg-gray-100 rounded"
                onClick={() => handleMenuOptionClick("Share")}
              >
